            ...
            steps{
                 script{
                        ...
                        withCredentials(bindings: [azureServicePrincipal('devServicePrincipal')]) {
                            ...
                            sh "az webapp config container set --name $WEBAPP_NAME --resource-group $RESOURCE_GROUP --docker-custom-image-name santiagoescalante.azurecr.io/frontend:v1.1 --docker-registry-server-url https://santiagoescalante.azurecr.io --docker-registry-server-user $azureLog_USR --docker-registry-server-password $azureLog_PSW"
                        }
                 }
            }
        }
